...
export class PetsController {
  ...
  create(@Body() createPetDto: CreatePetDto) {
    ...
    try {
      ...
      let one = this.petsService.create(createPetDto);
  ...
  findAll(@Query("limit") limit: number, @Query("page") page: number) {
    ...
    try {
      ...
      let all = this.petsService.findAll(from, to)
      if (all.length > 0) {
  ...
  update(@Request() req) {
    try {
      ...
      let one = this.petsService.update(id, updatePetDto);
      if (one) {
  ...
  remove(@Param('id') id: string) {
    try {
      let one = this.petsService.remove(id);
      if (one) {
        return { statusCode: 200, response: one._id }
      }
      throw new HttpException("Not found", HttpStatus.NOT_FOUND)
