...
export class AdoptionsController {
  ...
  @Post(":uid/:pid")
  create(@Param("uid") uid: string, @Param("pid") pid: string) {
    try {
      let _id = randomBytes(12).toString("hex")
      let createAdoptionDto: CreateAdoptionDto = { _id, owner: uid, pet: pid }
      let one = this.adoptionsService.create(createAdoptionDto);
  ...
  findAll(@Query("limit") limit: number, @Query("page") page: number) {
    ...
    try {
      ...
      let all = this.adoptionsService.findAll(from, to)
      if (all.length > 0) {
  ...
  findOne(@Param('aid') aid: string) {
    ...
    //con el tipado definido
    try {
      let one = this.adoptionsService.findOne(aid);
      if (one) {
